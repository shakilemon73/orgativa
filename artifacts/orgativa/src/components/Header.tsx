    ...
    <header style={{
      ...
      {isMobile ? (
        <>
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 16px" }}>
            <a href="/" onClick={(e) => { e.preventDefault(); navigate("/"); }}
              style={{ display: "flex", alignItems: "center", gap: 8, textDecoration: "none", userSelect: "none" }}>
              <div style={{ width: 32, height: 32, backgroundColor: P, borderRadius: "10px 3px 10px 3px", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                <span className="material-symbols-outlined" style={{ fontSize: 18, color: "#fff" }}>eco</span>
              </div>
              <div>
                <span style={{ fontFamily: "'Noto Serif',serif", fontSize: 18, fontWeight: 700, color: "#0D1F0B", letterSpacing: "-0.02em", lineHeight: 1 }}>অর্গাটিভা</span>
                <p style={{ fontSize: 8, color: "#8FA888", textTransform: "uppercase", letterSpacing: "0.15em", margin: 0, fontFamily: "'Inter',sans-serif", fontWeight: 600 }}>বিশুদ্ধ অর্গানিক</p>
              </div>
            </a>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <button onClick={() => {}} aria-label="পছন্দতালিকা"
                style={{ width: 36, height: 36, borderRadius: 8, border: "none", backgroundColor: "transparent", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}>
                <span className="material-symbols-outlined" style={{ fontSize: 22, color: "#4A5548" }}>favorite_border</span>
              </button>
              <button onClick={() => navigate("/cart")} aria-label={`ঝুড়ি, ${totalItems}টি পণ্য`}
                style={{ position: "relative", width: 40, height: 40, borderRadius: 10, backgroundColor: totalItems > 0 ? P : "#F4F7F3", border: "none", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}>
                <span className="material-symbols-outlined" style={{ fontSize: 22, color: totalItems > 0 ? "#fff" : "#4A5548" }}>shopping_basket</span>
                {totalItems > 0 && (
                  <span style={{ position: "absolute", top: -4, right: -4, backgroundColor: "#E63946", color: "#fff", fontSize: 9, fontWeight: 800, width: 16, height: 16, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Inter',sans-serif", border: "1.5px solid #fff" }}>
          ...
          <div style={{ padding: "0 16px 12px" }}>
            <div style={{
              ...
              boxShadow: searchFocused ? "0 0 0 3px rgba(45,90,39,0.1)" : "none",
              transition: "all 0.2s",
            }}>
              <span className="material-symbols-outlined" style={{ position: "absolute", left: 12, fontSize: 19, color: searchFocused ? P : "#A5B3A3" }}>search</span>
              <input type="text" placeholder="পণ্য খুঁজুন…"
                onFocus={() => setSearchFocused(true)} onBlur={() => setSearchFocused(false)}
                style={{ width: "100%", padding: "11px 12px 11px 42px", background: "transparent", border: "none", fontSize: 14, fontFamily: "'Inter',sans-serif", color: "#1A1C1C", outline: "none", boxSizing: "border-box" }} />
            </div>
          </div>
        </>
      ) : (
        ...
        <div style={{ maxWidth: 1280, margin: "0 auto", padding: `0 ${px}` }}>
          <div style={{ display: "flex", alignItems: "center", gap: isTablet ? 12 : 20, height: 72 }}>
            <a href="/" onClick={(e) => { e.preventDefault(); navigate("/"); }}
            ...
            <div style={{
              ...
              <span className="material-symbols-outlined" style={{ position: "absolute", left: 13, fontSize: 20, color: searchFocused ? P : "#A5B3A3", transition: "color 0.2s", pointerEvents: "none" }}>search</span>
              ...
              {!isTablet && <span style={{ position: "absolute", right: 12, fontSize: 10, color: "#A5B3A3", fontFamily: "'Inter',sans-serif", backgroundColor: "#E8EDE7", padding: "2px 7px", borderRadius: 4, fontWeight: 600, display: searchFocused ? "none" : "block" }}>⌘K</span>}
            </div>

            <div style={{ flex: 1 }} />

            <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
              {!isTablet && <ActionButton icon="favorite_border" label="পছন্দতালিকা" onClick={() => {}} />}
              ...
              <button onClick={() => navigate("/cart")} aria-label={`ঝুড়ি, ${totalItems}টি পণ্য`}
                style={{
                  ...
                  borderRadius: 12, padding: isTablet ? "8px 12px" : "9px 16px 9px 12px",
      ...
      {/* ── NAV STRIP (all sizes, horizontal scroll on mobile) ── */}
      <div style={{ backgroundColor: P, borderTop: "1px solid rgba(255,255,255,0.08)" }}>
        <div style={{ maxWidth: isMobile ? "none" : 1280, margin: "0 auto", padding: isMobile ? "0" : `0 ${px}` }}>
          <nav>
            <ul className="scroll-x" style={{ display: "flex", alignItems: "center", gap: 0, padding: isMobile ? "0 8px" : 0, margin: 0, listStyle: "none" }}>
              <li style={{ flexShrink: 0 }}>
                <button onClick={() => navigate("/category/all")}
                  style={{ display: "flex", alignItems: "center", gap: 5, color: "#fff", fontSize: isMobile ? 11 : 12, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em", background: "rgba(255,255,255,0.14)", border: "none", cursor: "pointer", padding: isMobile ? "10px 12px" : "11px 18px", fontFamily: "'Inter',sans-serif", whiteSpace: "nowrap", transition: "background 0.15s", borderRight: "1px solid rgba(255,255,255,0.1)" }}
              ...
              {navLinks.map((link) => (
                <NavItem key={link.slug} link={link} currentPath={location} isMobile={isMobile} />
              ))}
              {!isMobile && (
...
function ActionButton({ icon, label, onClick }: { icon: string; label: string; onClick: () => void }) {
  ...
  return (
    <button aria-label={label} onClick={onClick}
      ...
      style={{ width: 40, height: 40, borderRadius: 10, border: "1.5px solid", borderColor: hovered ? "#C2D9BC" : "transparent", backgroundColor: hovered ? P_LIGHT : "transparent", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", transition: "all 0.2s" }}>
      <span className="material-symbols-outlined" style={{ fontSize: 21, color: hovered ? P : "#4A5548", transition: "color 0.2s" }}>{icon}</span>
    </button>
  );
}

function NavItem({ link, currentPath, isMobile }: { link: typeof navLinks[0]; currentPath: string; isMobile: boolean }) {
  const [hovered, setHovered] = useState(false);
  const [, navigate] = useLocation();
  ...
  return (
    <li style={{ flexShrink: 0 }}>
      <a href={`/category/${link.slug}`}
        ...
        onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}
        style={{ display: "flex", alignItems: "center", gap: 4, color: active ? "#fff" : "rgba(255,255,255,0.75)", fontSize: isMobile ? 11 : 12, fontWeight: active ? 700 : 500, textTransform: "uppercase", letterSpacing: "0.06em", textDecoration: "none", padding: isMobile ? "10px 10px" : "11px 14px", backgroundColor: active ? "rgba(255,255,255,0.18)" : hovered ? "rgba(255,255,255,0.1)" : "transparent", borderBottom: active ? "2px solid #9ACA94" : "2px solid transparent", transition: "all 0.15s", whiteSpace: "nowrap", fontFamily: "'Inter',sans-serif" }}>
        <span className="material-symbols-outlined" style={{ fontSize: 13, opacity: 0.8 }}>{link.icon}</span>
        {link.label}
      </a>
    </li>
  );
